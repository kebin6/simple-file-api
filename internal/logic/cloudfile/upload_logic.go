...
import (
	...
	"github.com/kebin6/simple-file-api/internal/utils/filex"
...
func (l *UploadLogic) Upload() (resp *types.CloudFileInfoResp, err error) {
	...
	if provider == "" {
		return nil, errorx.NewCodeInternalError("file.nonDefaultProviderError")
	}
	...
	path := fmt.Sprintf("%s/%s/%s/%s",
		...
		fileType,
		storeFileName)
	url, err := l.UploadToProvider(file, path, provider)
	if err != nil {
		return nil, err
	}
	...
	return &types.CloudFileInfoResp{
		...
		Data: types.CloudFileInfo{
			...
			Url:      pointy.GetPointer(l.svcCtx.CloudStorage.ProviderData[provider].PreviewHost + data.Path),
