...
func (spc *StorageProviderCreate) SetPreviewHost(s string) *StorageProviderCreate {
	spc.mutation.SetPreviewHost(s)
	return spc
}
...
func (spc *StorageProviderCreate) check() error {
	...
	if _, ok := spc.mutation.PreviewHost(); !ok {
		return &ValidationError{Name: "preview_host", err: errors.New(`ent: missing required field "StorageProvider.preview_host"`)}
	}
